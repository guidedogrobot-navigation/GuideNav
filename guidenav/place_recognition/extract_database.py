...
from guidenav.models import pr_models as extractors
from guidenav.models.pr_models.base_model import dynamic_load
from guidenav.utils import read_image
...
def main(
    ...
    for idx, data in enumerate(tqdm(loader)):
        ...
        pred = model({"image": data["image"].to(device, non_blocking=True)}) # non-bolcking=True for faster transfer to GPU
        pred = {k: v[0].cpu().numpy() for k, v in pred.items()}

        pred["image_size"] = data["original_size"][0].numpy() # global_deiscriptor [D], score, image size [H, W]

        if as_half: # if half, half precision is used (float32 -> float16)
            for k in pred:
                dt = pred[k].dtype
