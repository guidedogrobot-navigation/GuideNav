...
# OUTPUT_DIR = "/media/hochul/2TB/IJRR_Topomaps/topo_gail2library_1m_topogen_dinov3"
...
# # Load DINOv3 model via torch hub (local weights)
...
# model.to(device)
# model.eval()
# print(f"Loaded {model_name} with local weights")

# # DINOv3 preprocessing transforms (for torch hub models)
...
#     # Preprocess image (torch hub models use standard transforms)
...
#         print(f"Saved keyframe: {save_path} (similarity: {cosine_similarity([feat], [last_feat])[0][0] if last_feat is not None else 'N/A'})")
#         last_feat = feat
#         key_idx += 1
...
# print(f"Keyframe extraction complete! Selected {key_idx} keyframes from {len(image_paths)} total images.")
